...
def proc_iter_data(datadir, common_dir):
    flag = True
    ...
    for z in range(1, 3000):
        ...
        if (flag):
            ...
            if not os.path.exists(common_dir + '/edge.pkl'):
                with open(common_dir + '/edge.pkl', 'wb') as fp:
                    pickle.dump(edge_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
            os.system(f"ln -s {common_dir + '/edge.pkl'} {outdir}")
        ...
        if (flag):
            file_name_list = ['/sizdata_x', '/sizdata_y', '/pdata']
            array_list = [trusize_x, trusize_y, pincnt]
            for file_name,array in zip(file_name_list, array_list):
                if not os.path.exists(common_dir + "/" + file_name + ".npy"):
                    np.save(common_dir + "/" + file_name + ".npy", array)
                os.system(f"ln -s {common_dir + '/' + file_name + '.npy'} {outdir}")
