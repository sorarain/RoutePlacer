...
if __name__ == '__main__':
    ...
    for netlist_name in netlist_names:
        netlist_dir = os.path.join(DATA_DIR,netlist_name)
        if not os.path.exists(netlist_dir):
            continue
        for i in range(2000):
            netlist_data_dir = os.path.join(netlist_dir,str(i))
            if not os.path.exists(netlist_data_dir):
                continue
            process(netlist_data_dir, args.binx, args.biny, args.cong, args.nctu, args.mask, args.initial, args.norm,
                    args.setzeromean, args.lower)
            proc_iter_data(netlist_data_dir, os.path.join(netlist_dir,"common"))
